...
//void RungeKutta(double (*f1)(double t, double y1, double y2), double(*f2)(double t, double y1, double y2), double* y1, double* y2, double h, double n, double* t, double y1_0, double y2_0) {
//    y1[0] = y1_0;
//    y2[0] = y2_0;
//    double k11, k12, k21, k22;
//    for (int i = 0; i < n; i++) {
//        k11 = f1(t[i], y1[i], y2[i]);
//        k12 = f2(t[i], y1[i], y2[i]);
//
//        k21 = f1(t[i] + h, y1[i] + h * k11, y2[i] + h * k12);
//        k22 = f2(t[i] + h, y1[i] + h * k11, y2[i] + h * k12);
//
//        y1[i + 1] = y1[i] + h * (k11 + k21) / 2;
//        y2[i + 1] = y2[i] + h * (k12 + k22) / 2;
//    }
...
void RungeKutta(double (**f)(double t, double* y), double** y, double h, double n, double* t, double* y0) {
    int count = 2;
...
void RungeKutta(int count, double (**f)(double t, double* y), double** y, double h, double n, double* t, double* y0) {
    for (int i = 0; i < count; i++) {
        y[0][i] = y0[i];
    }
    double* k1 = new double[count]();
...
int main()
{
    ...
    y0[1] = testY2(t[0]);
    std::cout << y0[0] << " " << y0[1] << "\n";
    //RungeKutta(TestF1, TestF2, y, h, n, t, testY1(t[0]), testY2(t[0]));
    //RungeKutta(f, y, h, n, t, y0);
    RungeKutta(2, f, y, h, n, t, y0);
    for (int i = 0; i < n; i++) {
        double testy1 = testY1(t[i]);
        double testy2 = testY2(t[i]);
        //std::cout << y1[i] << " " << testy1 << " " << y2[i] << " " << testy2 << " " << y1[i] - testy1 << " " << y2[i] - testy2 << "\n";
